...
function index() {
	...
	if ($user_index->getUid() !== NULL) {
		...
		echo str_replace(
				array('[LEFT]', '[TOP]', '[EXECUTIVELIST]', '[CONTRACTLIST]',
						'[COUNT_EXECUTIVE]', '[COUNT_CONTRACT_CUS]',
						'[ALLCOUNT]', '[INVOICETAB]', '[INVOICELIST]',
						'[DEPOSITTAB]', '[DEPOSITLIST]',
						'[SUPPLIERAPPYAUDITTAB]', '[SUPPLIERAPPYAUDITLIST]',
						'[CONTRACTPAYMENTPERSONTAB]',
						'[CONTRACTPAYMENTPERSONLIST]', '[PAYMENTMESSAGE]',
						'[OUTSOURCINGAUDITTAB]', '[OUTSOURCINGAUDITLIST]',
						'[BASE_URL]'),
				array($user_index->get_left_html(),
						...
						$user_index->getOutsourcingAuditList(), BASE_URL),
				$buf);
	} else {
		session_unset();
		session_destroy();
