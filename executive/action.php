	...
	if ($result['status'] === 'error') {
		...
		if (strval(Security_Util::my_post('action')) === 'executive_alter') {
			Js_Util::my_show_success_message($result['message'],
					BASE_URL . 'executive/?o=mylist');
		} else if (in_array(Security_Util::my_post('action'),
				array('executive_userchange', 'executive_userchange_all'),
				TRUE)) {
			Js_Util::my_show_success_message($result['message'],
					BASE_URL . 'executive/?o=userchange');
		} else {
			Js_Util::my_show_success_message($result['message'],
					(in_array(strval(Security_Util::my_post('action')),
							array('executive_audit', 'executive_dep_edit',
									'executive_dep_audit', 'executive_update',
									'executive_complete_cy'), TRUE) ? BASE_URL
							: NULL));
		}
	}
} else {
		...
		if (!empty($ccc)) {
			...
			foreach ($cycount as $cy) {
				if (!empty($cy)) {
					...
					if ($isaltershow === 0 && !$exe_isalter
							&& (strtotime($date) < strtotime($start)
					...
					$cyitem[$ccc][] = array(
							'date' => Security_Util::my_post(
									'cyym_' . $ccc . '_' . $cy),
							'cost' => Security_Util::my_post(
									'cost_amount_' . $ccc . '_' . $cy));
					$cost += Security_Util::my_post(
							'cost_amount_' . $ccc . '_' . $cy);
				}
			}
			...
			if ($dateError) {
				echo '<script>';
				echo 'parent.show_message("项目执行日期为' . $start . '至' . $end
						. '，拆月数据必须在该日期范围内");';
				echo '</script>';
				exit;
			...
			$all[$ccc] = array(
					...
					'short_id' => Security_Util::my_post(
							'supplier_short_' . $ccc), 'items' => $cyitem,
					'cost' => $cost,
...
function executive_do($action) {
	if ($action === 'audit') {
		$fields = array('audit_pass' => Security_Util::my_post('audit_pass'),
		...
		if (!in_array($action,
				array('dep_edit', 'dep_audit', 'userchange', 'userchange_all'),
				TRUE)) {
			//合同约定付款时间 
		...
		if (!in_array($action,
				array('dep_audit', 'userchange', 'userchange_all'), TRUE)) {
			//服务金额拆分
			...
			foreach ($servicecf as $scf) {
				if (!empty($scf)) {
					$servicecf_array[] = array(
							'type' => Security_Util::my_post(
									'servercf_type_' . $scf),
							'amount' => Security_Util::my_post(
									'servercf_amount_' . $scf),
							'remark' => Security_Util::my_post(
									'servercf_remark_' . $scf));
				}
			}
		...
		if ($action === 'dep_edit') {
			$fields = array('actor' => Security_Util::my_post('actor'),
					...
					'outsourcing_type' => Security_Util::my_post(
							'outsourcing_type') //执行单外包类型
			);
		} else if ($action === 'dep_audit') {
			$fields = array('dep' => Security_Util::my_post('dep'),
					'audit_pass' => Security_Util::my_post('audit_pass'),
					'remark' => Security_Util::my_post('remark'));
		} else if ($action === 'userchange') {
			$fields = array('user' => Security_Util::my_post('user'),
					'principal' => Security_Util::my_post('principal'));
		} else if ($action === 'userchange_all') {
			$fields = array('user' => Security_Util::my_post('type1users'),
					'principal' => Security_Util::my_post('type1principal'));
		} else {
			$fields = array('exetype' => Security_Util::my_post('exetype'),
					...
					'support_array' => Security_Util::my_checkbox_post(
							'support'),
					'process' => Security_Util::my_post('process'),
					'remark' => Security_Util::my_post('remark'),
					...
					'altersupport_array' => Security_Util::my_checkbox_post(
							'altersupport'),
					'cy_json' => Security_Util::my_post('cy_json'),
	...
	if ($action === 'update' || $action === 'audit' || $action === 'dep_audit'
			|| $action === 'dep_edit') {
		$fields['executive_id'] = Security_Util::my_post('executive_id');
	}

	if (in_array($action,
			array('audit', 'dep_edit', 'dep_audit', 'alter', 'update',
					'userchange', 'userchange_all'), TRUE)) {
...
function executive_complete_cy() {
	...
	$fields = array('executive_id' => Security_Util::my_post('executive_id'),
