...
class FileTypeValidation {
	...
	public static function validation($filePath, $fileExt) {
		...
		foreach ( $val as $v ) {
			...
			// 判断文件头
			if (strtolower ( $v ) == $fileHead [1]) {
				if (in_array ( $fileExt, array ('jpg', 'png', 'gif' ), TRUE )) {
	 ...
	 * @return string
	 */
	private static function _readFile($filePath, $size) {
