<?php
class Deposit_Receivables extends User {
	private $id;
	private $type;
	private $page;
	private $search;
	...
	private function _getNone1() {
		return $this->has_deposit_receivables_permission ? 'none' : '';
	}

	private function _getNone2() {
		return $this->has_deposit_receivables_permission ? '' : 'none';
	}
	...
	public function __construct($fields = array()) {
		...
		if (!empty($fields)) {
			foreach ($this as $key => $value) {
				if ($fields[$key] !== NULL
						&& !in_array($key,
								array('has_deposit_receivables_permission'),
								TRUE)) {
					$this->$key = $fields[$key];
				}
			}
		...
		if (intval($this->finance_receivables_id) > 0) {
			$row = $this->db
					->get_row(
							'SELECT a.cid,FROM_UNIXTIME(b.time) AS tt,c.cusname,a.amount,b.payer,c.city,c.dep,b.date,a.isok,d.username,d.realname FROM  finance_deposit_receivables a LEFT JOIN finance_deposit_receivables_list b ON a.receivables_list=b.id LEFT JOIN contract_cus c ON a.cid=c.cid LEFT JOIN users d ON a.user=d.uid WHERE a.id='
	...
	public function get_add_deposit_receivables_html() {
		if ($this->has_deposit_receivables_permission) {
			$buf = file_get_contents(
					TEMPLATE_PATH . 'finance/deposit/deposit_receivables.tpl');
			return str_replace(
					array('[LEFT]', '[TOP]', '[VCODE]', '[NONE1]', '[NONE2]',
							'[BASE_URL]'),
					array($this->get_left_html(), $this->get_top_html(),
							$this->get_vcode(), $this->_getNone1(),
							$this->_getNone2(), BASE_URL), $buf);
		} else {
			return User::no_permission();
		}
	}

	public function get_search_deposit_html() {
		if ($this->has_deposit_receivables_permission) {
			return $this->_search_deposit();
		} else {
			return NO_RIGHT_TO_DO_THIS;
		}
	}

	private function _get_receivables_amount($cid) {
		$amount = $this->db
				->get_var(
						'SELECT SUM(amount) FROM finance_deposit_receivables WHERE cid="'
								. $cid . '" AND isok=1');
		if ($amount === NULL) {
			return 0;
		}
	...
	private function _get_invoice_amount($cid) {
		$amount = $this->db
				->get_var(
	...
	private function _search_deposit() {
		$s = '';
		if (intval($this->type) === 1) {
			$sql = 'SELECT COUNT(*) FROM (SELECT DISTINCT(a.cid) FROM finance_deposit_invoice a LEFT JOIN finance_deposit_invoice_list b ON a.invoice_list_id=b.id WHERE b.number LIKE "%'
					. $this->search . '%") z';
		} else {
			$sql = 'SELECT COUNT(*) FROM finance_deposit a LEFT JOIN contract_cus b ON a.cid=b.cid WHERE b.isok=1 AND (b.cusname LIKE "%'
					. $this->search . '%" OR b.cid LIKE "%' . $this->search
					. '%")';
		}
		...
		if ($all_count > 0) {
			if (intval($this->type) === 1) {
				$sql = 'SELECT z.cid,y.amount,x.cusname FROM (SELECT DISTINCT(a.cid) FROM finance_deposit_invoice a LEFT JOIN finance_deposit_invoice_list b ON a.invoice_list_id=b.id WHERE b.number LIKE "%'
						. $this->search
						. '%") z ,finance_deposit y,contract_cus x WHERE z.cid=y.cid AND y.cid=x.cid AND y.isok>=0 ORDER BY z.cid LIMIT '
						. $start . ',' . self::EXE_LIMIT;
			} else {
				$sql = 'SELECT a.* FROM finance_deposit a LEFT JOIN contract_cus b ON a.cid=b.cid WHERE b.isok=1 AND  (b.cusname LIKE "%'
						. $this->search . '%" OR b.cid LIKE "%' . $this->search
						. '%" ) ORDER BY a.cid LIMIT ' . $start . ','
						. self::EXE_LIMIT;
			}
			//var_dump($sql);
			$exes = $this->db->get_results($sql);
			$s .= '<div><span style="display:inline-block;width:60px;">&nbsp;&nbsp;</span><span style="display:inline-block;width:90px;"><b>合同号</b></span>&nbsp;&nbsp;<span style="display:inline-block;width:300px;"><b>客户名称</b></span>&nbsp;&nbsp;<span style="display:inline-block;width:80px;"><b>总金额</b></span>&nbsp;&nbsp;<span style="display:inline-block;width:80px;"><b>已收账款</b></span>&nbsp;&nbsp;<span style="display:inline-block;width:80px;"><b>应收账款</b></span>&nbsp;&nbsp;<span style="display:inline-block;width:80px;"><b>已开票总金额</b></span></div>';
			foreach ($exes as $key => $exe) {
				$receivable_amount = $this->_get_receivables_amount($exe->cid);
				$done_invoice = $this->_get_invoice_amount($exe->cid);
				...
				if ($details !== NULL) {
					...
					foreach ($details as $detail) {
						$s .= '<tr><td>'
								. Format_Util::my_money_format('%.2n',
										$detail->amount) . '</td><td>'
								. $detail->number . '</td><td>' . $detail->date
								. '</td></tr>';
					}
					$s .= '</table></div>';
					$s .= '</div>';
	...
	public function getHas_deposit_receivables_permission() {
		return $this->has_deposit_receivables_permission;
	}
	...
	private function _check_cid_amount($cid, $amount) {
		$row = $this->db
				->get_row(
						'SELECT isok,amount FROM finance_deposit WHERE cid="'
								. $cid . '" FOR UPDATE');
		if ($row === NULL) {
			//没有该保证金
			return FALSE;
	...
	public function add_deposit_receivables() {
		if ($this->validate_form_value('add')) {
			...
			foreach ($pids_array as $pid => $amount) {
				$check_result = $this->_check_cid_amount($pid, $amount);
				if (!$check_result) {
					$check_all = FALSE;
					break;
				}
			...
			if ($success) {
				foreach ($pids_array as $pid => $amount) {
					...
					$insert_result = $this->db
							->query(
									'INSERT INTO finance_deposit_receivables(cid,amount,user,time) VALUE("'
											. $pid . '","' . $amount . '",'
											. $this->getUid() . ','
			...
			if ($success && !empty($ids)) {
				$insert_result = $this->db
						->query(
								'INSERT INTO finance_deposit_receivables_list(date,amount,receivables_ids,content,user,time,payer) VALUE("'
			...
			if ($success && $insert_id > 0) {
				$update_result = $this->db
						->query(
								'UPDATE finance_deposit_receivables SET receivables_list='
	...
	private function _get_pid_list($contents) {
		$s = '';
		if (!empty($contents)) {
			...
			foreach ($contents as $content) {
				...
				$row = $this->db
						->get_row(
								'SELECT a.cid,a.amount,b.cusname FROM finance_deposit a LEFT JOIN contract_cus b ON a.cid=b.cid WHERE a.cid="'
										. $content[0] . '"');
				if ($row !== NULL) {
					$receivable_amount = $this
							->_get_receivables_amount($row->cid);
					$tmps = '<td>' . $row->cid . '</td><td>' . $row->cusname
					...
					$s .= '<tr><td><img src="' . BASE_URL
							...
							. $content[0]
							. '\')" width="12" height="12" />&nbsp;<input onblur="javascript:checksum();" type="text" class="validate[required,custom[invoiceMoney]] text" style="width:100px; height:12px" name="amount1_'
			...
			if ($s !== '') {
				$s .= '<tr><td><b>合计</b></td><td colspan="3" id="sumshow"><font color="red">'
						. $allamount . '</font></tr>';
			}
		}
	...
	private function _get_pids($contents) {
		if (!empty($contents)) {
			$arr = array();
			$contents = explode('|', $contents);
			foreach ($contents as $content) {
	...
	public function get_edit_deposit_receivables_html() {
		if ($this->id !== NULL) {
			if ($this->has_deposit_receivables_permission) {
				$row = $this->db
						->get_row(
								'SELECT * FROM finance_deposit_receivables_list WHERE id='
										. intval($this->id) . ' AND isok=1');
				if ($row !== NULL) {
					$buf = file_get_contents(
							TEMPLATE_PATH
									. 'finance/deposit/deposit_receivables_edit.tpl');
					return str_replace(
							...
							array($this->get_left_html(),
									...
									$this->_getNone1(), $this->_getNone2(),
									$row->date, $row->amount, $row->payer,
									$this->_get_pid_list($row->content),
									$this->_get_pids($row->content), $this->id,
									BASE_URL), $buf);
				} else {
	...
	public function update_deposit_receivables() {
		if ($this->validate_form_value('update')) {
			...
			$delete_result = $this->db
					->query(
							'DELETE FROM finance_deposit_receivables WHERE receivables_list='
			...
			if ($delete_result !== FALSE) {
				...
				if ($success) {
					foreach ($pids_array as $pid => $amount) {
						...
						$insert_result = $this->db
								->query(
										'INSERT INTO finance_deposit_receivables(cid,receivables_list,amount,user,time) VALUE("'
												. $pid . '","'
												. intval($this->update_id)
												. '","' . $amount . '",'
												. $this->getUid() . ','
				...
				if ($success && !empty($ids)) {
					$update_result = $this->db
							->query(
									'UPDATE finance_deposit_receivables_list SET date="'
											. $this->date . '",amount="'
											. $this->amount
											. '",receivables_ids="'
											. implode('^', $ids)
	...
	private function _cancel() {
		if (self::validate_id(intval($this->id))) {
			...
			$update_result = $this->db
					->query(
							'UPDATE finance_deposit_receivables_list SET isok=-1 WHERE id='
									. intval($this->id));
			if ($update_result === FALSE) {
				$success = FALSE;
				...
				$update_result = $this->db
						->query(
								'UPDATE finance_deposit_receivables SET isok=-1 WHERE receivables_list='
										. intval($this->id));
				if ($update_result === FALSE) {
					$success = FALSE;
	...
	public function cancel_deposit_receivable() {
		if ($this->has_deposit_receivables_permission) {
			$result = $this->_cancel();
	...
	public function get_import_deposit_receivables_html() {
		if ($this->has_deposit_receivables_permission) {
			$buf = file_get_contents(
					TEMPLATE_PATH
							. 'finance/deposit/deposit_receivables_import.tpl');
			return str_replace(
	...
	private function _check_format($line, $infos, &$errors) {
		...
		//第四列 执行单号
		if (!$this->_check_cid_amount($infos[3], $infos[1])) {
				...
				for ($i = 2; $i <= $sum_rows_count; $i++) {
					...
					if ($isok) {
						...
						if ($result1 === FALSE) {
							...
							$result2 = $this->db
									->query(
											'INSERT INTO finance_deposit_receivables_list(date,amount,receivables_ids,content,user,time,payer,isexport) VALUE("'
													. date('Y-m-d',
