<?php
class Invoice_Search extends User {
	private $starttime;
	private $endtime;
	private $search;
	private $page;
	...
	public function __construct($starttime = NULL, $endtime = NULL,$search = NULL) {
		parent::__construct();
	...
	public function _get_list_html() {
		...
		if ($this->has_invoice_permission) {
			$query = 'SELECT a.*,FROM_UNIXTIME(a.gdtime) AS tt,b.username,b.realname,c.username AS gbusername,c.realname AS gdrealname FROM finance_invoice_list a LEFT JOIN users b ON a.user=b.uid LEFT JOIN users c ON a.gduser=c.uid WHERE a.isok=1';
			...
			if($this->search !== NULL && $this->search !== ''){
				$query .= ' AND (a.title LIKE "%' . $this->search . '%" OR a.number LIKE "%' . $this->search . '%")';
			}
				...
				foreach ($results as $key => $result) {
					$s .= '<tr><td><input type="checkbox" class="validate[minCheckbox[1]]" name="selinvoice[]" value="'
							...
							. Format_Util::my_money_format('%.2n',
									$result->amount) . '</b></font></td><td>' . $result->number . '</td><td>'
							. Invoice_List::get_invoice_type(
									intval($result->type)) . '</td><td>'
							. $result->realname . ' (' . $result->username
							. ')' . '</td><td>' . ($result->gbusername !== NULL && $result->gdrealname !== NULL ? ($result->gdrealname . ' (' . $result->gbusername . ')') : '') . '</td><td>'
							. Invoice_List::get_invoice_action(
									intval($result->id), 0,
									intval($result->print),
