<?php
class Finance_Receivables_Search extends User {
	private $starttime;
	private $endtime;
	private $search;
	...
	private function _get_list_html() {
		$s = '';
		if ($this->has_finance_receivables_permission) {
			$query = 'SELECT a.id,a.pid,FROM_UNIXTIME(b.time) AS tt,c.cusname,a.amount,b.payer,b.date,a.isok,d.username,d.realname FROM  finance_receivables a LEFT JOIN (SELECT SUBSTRING_INDEX(pid,"-",1) AS cid,id FROM finance_receivables ) z ON a.id=z.id LEFT JOIN finance_receivables_list b ON a.receivables_list=b.id LEFT JOIN contract_cus  c ON z.cid=c.cid LEFT JOIN users d ON a.user=d.uid WHERE 1=1';
			...
			if ($results !== NULL) {
				foreach ($results as $key => $result) {
					$s .= '<tr><td><input type="checkbox" class="validate[minCheckbox[1]]" name="selinvoice[]" value="'
							...
							. $result->payer . '</td><td>' . $result->realname . ' (' . $result->username . ')' . '</td><td>' . self::_get_status(intval($result->isok)) . '</td></tr>';
				}
			} else {
	...
	public function get_receivables_search_html() {
		if ($this->has_finance_receivables_permission) {
			$buf = file_get_contents(
					TEMPLATE_PATH
							. 'finance/receivables/finance_receivables_search.tpl');
			return str_replace(
