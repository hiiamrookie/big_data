...
class Finance_Report extends User {
		...
		if ($this->has_finance_report_permission) {
			...
			if($results !== NULL){
				foreach ($results as $result){
					$s .= '<tr><td width="5%">&nbsp;&nbsp;<input type="checkbox" name="seldate[]" value="' . $result->settle_account_date . '"></td><td>' . $result->settle_account_date . '</td></tr>';
				}
			}
			$buf = file_get_contents(
					TEMPLATE_PATH . 'finance/finance_report.tpl');
			return str_replace(
					array('[SETTLEACCOUNTDATES]','[LEFT]', '[TOP]', '[VCODE]', '[BASE_URL]'),
					array($s,$this->get_left_html(), $this->get_top_html(),
	...
	private function _getReportPids($exist_executive_ids = array(),
			$exist_pids = array()) {
		$executive_ids = array();
		$pids = array();
		$cids = array();
		$paytimes = array();
		if (empty($exist_executive_ids) && empty($exist_pids)) {
			$results = $this->db
					->get_results(
							'SELECT b.id,b.pid,b.cid,b.city,b.dep,b.team,b.name,b.type,b.amount,b.allcost,b.starttime,b.endtime,b.company,b.time,b.paytimeinfoids,b.isalter FROM (SELECT a.* FROM (SELECT id,pid,cid,city,dep,team,isalter,isok,oktime,name,type,amount,allcost,starttime,endtime,company,time,paytimeinfoids FROM executive WHERE isok<>-1 ORDER BY pid,isalter DESC) a GROUP BY pid) b WHERE b.isok=1 AND b.oktime>='
			...
			$results = $this->db
					->get_results(
							'SELECT b.id,b.pid,b.cid,b.city,b.dep,b.team,b.name,b.type,b.amount,b.allcost,b.starttime,b.endtime,b.company,b.time,b.paytimeinfoids,b.isalter FROM (SELECT a.* FROM (SELECT id,pid,cid,city,dep,team,isalter,isok,oktime,name,type,amount,allcost,starttime,endtime,company,time,paytimeinfoids FROM executive WHERE isok<>-1 AND pid IN ("'
									. implode('","', $exist_pids)
									. '") AND id NOT IN ('
									. implode(',', $exist_executive_ids)
									. ') ORDER BY pid,isalter DESC) a GROUP BY pid) b WHERE b.isok=1');
		}
		if ($results !== NULL) {
			$tmp_cids = array();
			$tmp_paytimes = array();
			foreach ($results as $result) {
				$executive_ids[] = $result->id;
				$pids[$result->pid] = array('executive_id' => $result->id,
						'pid' => $result->pid, 'cid' => $result->cid,
						'city' => $result->city, 'dep' => $result->dep,
						'team' => $result->team, 'name' => $result->name,
						'type' => $result->type, 'amount' => $result->amount,
				...
				if (!empty($paytimeinfoids)) {
					...
					foreach ($paytimeinfoids as $paytimeinfoid) {
						if (!empty($paytimeinfoid)) {
							$tmp_paytimes['relate'][$result->pid][] = (int) $paytimeinfoid;
	...
	private function _getAmountCY($executive_ids) {
		...
		if ($results !== NULL) {
			foreach ($results as $result) {
				//$cys[$result->ym][$result->executive_id] += $result->quote_amount;
				$cys[$result->ym][$result->pid] += $result->quote_amount;
			}
		}
		return $cys;
	}
...
ON a.industry_id=e.id');
		...
		if ($results !== NULL) {
			foreach ($results as $result) {
				$cys['cy'][$result->ym][$result->pid . '|'
						. $result->supplier_id . '|'
						. $result->supplier_short_id . '|'
						. $result->category_id . '|' . $result->industry_id
						. '|' . $result->is_support . '|'
						. $result->support_dep] = array(
				...
				$cys['cy_sum'][$result->ym][$result->pid] += $result
						->cost_amount;
			}
		}
		return $cys;
	...
	private function _getSettleAccountDate() {
		$settle_account_date = $this->db
				->get_var(
						'SELECT MAX(settle_account_date) AS settle_account_date FROM finance_settle_account');
		if (!empty($settle_account_date)) {
			//有结账日期
			$settle_account_date = date('Y-m', strtotime($settle_account_date));
		} else {
			$settle_account_date = NULL;
		}
		return $settle_account_date;
	}
	...
	public function get_finance_report() {
		if ($this->has_finance_report_permission) {
			...
			if (!empty($pids['pids'])) {
				$report_pids = $pids['pids'];

				$exist_executive_ids = $pids['executive_ids'];
				$exist_pids = array_keys($report_pids);

				//前一版本的执行单
				$prev_pids = $this
						->_getReportPids($exist_executive_ids, $exist_pids);

				$prev_report_pids = $prev_pids['pids'];
				...
				//成本拆月
				$cys = $this->_getCostCY($exist_executive_ids);
				$cys_n = $cys['cy'];

				//前一版本的成本拆月
				$prev_cys = $this->_getCostCY($prev_pids['executive_ids']);
				$prev_cys_n = $prev_cys['cy'];

				//金额拆月
				$amount_cys = $this->_getAmountCY($exist_executive_ids);

				//前一版本的金额拆月
				$prev_amount_cys = $this
						->_getAmountCY($prev_pids['executive_ids']);
				...
				if (!empty($cys_n)) {
					...
					//结帐日期
					$settleDate = $this->_getSettleAccountDate();
					if ($settleDate !== NULL
							&& strtotime($first_month) > strtotime($settleDate)) {
						$settleDate = NULL;
					}
					...
					foreach ($months as $key => $month) {
						...
						$now_pid = '';
						foreach ($contents as $nKey => $content) {
							...
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(3, $now_line,
											$pids['cids'][$report_pids[$content['pid']]['cid']]['cusname']);

							//所属公司
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(4, $now_line,
											self::_getCompany(
													$report_pids[$content['pid']]['company']));

							//所属发票
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(5, $now_line,
											self::_getBilltype(
													$pids['cids'][$report_pids[$content['pid']]['cid']]['billtype']));

							//直客
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(6, $now_line,
											self::_getCidCustomType(
													$pids['cids'][$report_pids[$content['pid']]['cid']]['type1']));
							...
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(8, $now_line,
											self::_getCampaign(
													$report_pids[$content['pid']]['name'],
													intval(
															$report_pids[$content['pid']]['isalter'])));

							//项目管理-客户
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(9, $now_line,
											self::_getDepInfo($dep, $team,
													$report_pids[$content['pid']]['dep'],
													$report_pids[$content['pid']]['team']));
							...
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(11, $now_line,
											self::_getPidType(
													$report_pids[$content['pid']]['type']));

							//投放时间
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(12, $now_line,
											self::_getTimeRange(
													$report_pids[$content['pid']]['starttime'],
													$report_pids[$content['pid']]['endtime']));

							//应收时间
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(13, $now_line,
											implode(',',
													$pids['paytimes'][$content['pid']]));

							if ($now_pid !== $content['pid']) {
								//同一执行单只显示第一行

								//执行总金额
								$prev_amount = $prev_report_pids[$prev_cys_n[$month][$nKey]['pid']]['amount'];
								$now_amount = $report_pids[$content['pid']]['amount'];
								$objPHPExcel->getActiveSheet()
										->setCellValueByColumnAndRow(14,
												$now_line,
												(!empty($prev_amount) ? ($now_amount
																- $prev_amount)
														: $now_amount));
								$objPHPExcel->getActiveSheet()
								...
								//当月执行金额
								$prev_amount_cy = $prev_amount_cys[$month][$content['pid']];
								$now_amount_cy = $amount_cys[$month][$content['pid']];
								$objPHPExcel->getActiveSheet()
										->setCellValueByColumnAndRow(15,
												$now_line,
												(!empty($prev_amount_cy) ? ($now_amount_cy
																- $prev_amount_cy)
														: $now_amount_cy));
								$objPHPExcel->getActiveSheet()
										...
										->setFormatCode(
												PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED3);
								unset($prev_amount_cy);
								unset($now_amount_cy);

								//执行总成本
								$prev_allcost = $prev_report_pids[$prev_cys_n[$month][$nKey]['pid']]['allcost'];
								$now_allcost = $report_pids[$content['pid']]['allcost'];
								$objPHPExcel->getActiveSheet()
										->setCellValueByColumnAndRow(16,
												$now_line,
												(!empty($prev_allcost) ? $now_allcost
																- $prev_allcost
														: $now_allcost));
								$objPHPExcel->getActiveSheet()
								...
								//当月执行成本
								$prev_cy_sum = $prev_cys['cy_sum'][$month][$content['pid']];
								$now_cy_sum = $cys['cy_sum'][$month][$content['pid']];
								$objPHPExcel->getActiveSheet()
										->setCellValueByColumnAndRow(17,
												$now_line,
												(!empty($prev_cy_sum) ? ($now_cy_sum
																- $prev_cy_sum)
														: $now_cy_sum));
								$objPHPExcel->getActiveSheet()
							...
							//当月执行成本明细
							$prev_cost_amount = $prev_cys_n[$month][$nKey]['cost_amount'];
							$now_cost_amount = $content['cost_amount'];
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(23, $now_line,
											(!empty($prev_cost_amount) ? ($now_cost_amount
															- $prev_cost_amount)
													: $now_cost_amount));
							$objPHPExcel->getActiveSheet()
									...
									->setFormatCode(
											PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED3);
							unset($prev_cost_amount);
							unset($now_cost_amount);
