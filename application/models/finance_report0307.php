...
class Finance_Report extends User {
	...
	public function get_report_html() {
		if ($this->has_finance_report_permission) {
			...
			$results = $this->db
					->get_results(
							'SELECT id,settle_account_date FROM finance_settle_account ORDER BY settle_account_date DESC');
			if ($results !== NULL) {
				foreach ($results as $result) {
					$s .= '<tr><td width="5%">&nbsp;&nbsp;<input type="checkbox" name="seldate[]" value="'
							. strtotime($result->settle_account_date)
							. '"></td><td>' . $result->settle_account_date
							. '</td></tr>';
				}
			}
			...
			return str_replace(
					array('[SETTLEACCOUNTDATES]', '[LEFT]', '[TOP]', '[VCODE]',
							'[BASE_URL]'),
...
ON a.industry_id=e.id ORDER BY a.ym,a.executive_id,a.supplier_id,a.supplier_short_id,a.category_id,a.industry_id,a.is_support,a.support_dep');

		if ($results !== NULL) {
			foreach ($results as $result) {
				$cys['cy'][$result->ym][$result->executive_id][$result
						->supplier_id . '|' . $result->supplier_short_id . '|'
	...
	private function _getPids() {
		...
		$results = $this->db
				->get_results(
						'SELECT b.id,b.pid,b.cid,b.city,b.dep,b.team,b.name,b.type,b.amount,b.allcost,b.starttime,b.endtime,b.company,b.time,b.paytimeinfoids,b.isalter,FROM_UNIXTIME(b.oktime) AS oktimeshow FROM (SELECT a.* FROM (SELECT id,pid,cid,city,dep,team,isalter,isok,oktime,name,type,amount,allcost,starttime,endtime,company,time,paytimeinfoids FROM executive WHERE isok<>-1 ORDER BY pid,isalter DESC) a GROUP BY pid) b WHERE b.isok=1 AND b.oktime>='
								. $this->starttime . ' AND b.oktime<='
								. $this->endtime);
		if ($results !== NULL) {
			foreach ($results as $result) {
				if (intval($result->isalter) !== 0) {
					//有变更的执行单，查找以前版本
					$prev_results = $this->db
							->get_results(
									'SELECT id,pid,cid,city,dep,team,isalter,isok,oktime,name,type,amount,allcost,starttime,endtime,company,time,paytimeinfoids,FROM_UNIXTIME(oktime) AS oktimeshow FROM executive WHERE pid="'
											. $result->pid . '" AND isalter<'
											. $result->isalter
											. ' AND isok=1 ORDER BY isalter DESC');
					if ($prev_results !== NULL) {
						$getOne = FALSE;
						foreach ($prev_results as $prev_result) {
							$oktime = $prev_result->oktime;
							if ($oktime >= $this->starttime
									|| !$getOne && $oktime < $this->starttime) {
								...
								$pids[$prev_result->id] = array(
										...
										'oktimeshow' => $prev_result
												->oktimeshow,
										'in_range' => ($oktime
												>= $this->starttime));

								if (!in_array($prev_result->cid, $tmp_cids,
										TRUE)) {
								...
								$prev_paytimeinfoids = $prev_result
										->paytimeinfoids;
								if (!empty($prev_paytimeinfoids)) {
									$prev_paytimeinfoids = explode('^',
											$prev_paytimeinfoids);
									foreach ($prev_paytimeinfoids as $prev_paytimeinfoid) {
										if (!empty($prev_paytimeinfoid)) {
											$tmp_paytimes['relate'][$prev_result
													->id][] = (int) $prev_paytimeinfoid;
				...
				$pids[$result->id] = array('executive_id' => $result->id,
		...
		return empty($pids) ? $pids
				: array('pids' => $pids, 'cids' => $cids,
						'paytimes' => $paytimes,
				...
				foreach ($cc as $exe_id => $cost) {
					foreach ($cost as $nkey => $cost_array) {
						if (count($cost_array) > 1) {
							...
							$data['cost_cy'][$month][$exe_id][$nkey] = $cost_array[0];
						}
					}
				}
			}
	...
	public function get_finance_report() {
		if ($this->has_finance_report_permission) {
			...
			if (!empty($data)) {
				$months = array_unique(
						array_merge(array_keys($data['cost_cy']),
								array_keys($data['amount_cy_sum'])));
				sort($months);
				...
				foreach ($months as $key => $month) {
					...
					$objPHPExcel->getActiveSheet()
							->setCellValueByColumnAndRow(0, 1, '媒体投放量汇总');
					...
					foreach ($data_cost_cy[$month] as $executive_id => $contents) {
						...
						foreach ($contents as $nKey => $recs) {
							...
							$rebid = $recs['supplier_id'] . '|'
									. $recs['supplier_short_id'] . '|'
									. $recs['category_id'] . '|'
									. $recs['industry_id'];
							...
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(1, $now_line,
											date('Y-m-d H:i:s',
													$pids['pids'][$executive_id]['time']));

							//执行单号
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(2, $now_line,
											$pids['pids'][$executive_id]['pid']);

							//客户
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(3, $now_line,
											$pids['cids'][$pids['pids'][$executive_id]['cid']]['cusname']);

							//所属公司
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(4, $now_line,
											self::_getCompany(
													$pids['pids'][$executive_id]['company']));

							//所属发票
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(5, $now_line,
											self::_getBilltype(
													$pids['cids'][$pids['pids'][$executive_id]['cid']]['billtype']));

							//直客
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(6, $now_line,
											self::_getCidCustomType(
													$pids['cids'][$pids['pids'][$executive_id]['cid']]['type1']));
							...
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(8, $now_line,
											self::_getCampaign(
													$pids['pids'][$executive_id]['name'],
													intval(
															$pids['pids'][$executive_id]['isalter'])));

							//项目管理-客户
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(9, $now_line,
											self::_getDepInfo($dep, $team,
													$pids['pids'][$executive_id]['dep'],
													$pids['pids'][$executive_id]['team']));
							...
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(11, $now_line,
											self::_getPidType(
													$pids['pids'][$executive_id]['type']));

							//投放时间
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(12, $now_line,
											self::_getTimeRange(
													$pids['pids'][$executive_id]['starttime'],
													$pids['pids'][$executive_id]['endtime']));

							//应收时间
							$objPHPExcel->getActiveSheet()
									->setCellValueByColumnAndRow(13, $now_line,
											implode(',',
													$pids['paytimes'][$executive_id]));
							...
							if ($now_pid_isalter
								...
								$data_recounts[$pids['pids'][$executive_id]['pid']][$pids['pids'][$executive_id]['isalter']][$rebid
										. '|' . $recs['is_support'] . '|'
										. $recs['support_dep']]['amount'] = array(
										'row' => $now_line, 'col' => 14,
								...
								$data_recounts[$pids['pids'][$executive_id]['pid']][$pids['pids'][$executive_id]['isalter']][$rebid
										. '|' . $recs['is_support'] . '|'
										. $recs['support_dep']]['now_month_amount'] = array(
										'row' => $now_line, 'col' => 15,
								...
								$objPHPExcel->getActiveSheet()
										->setCellValueByColumnAndRow(16,
												$now_line,
												$pids['pids'][$executive_id]['allcost']);
								$objPHPExcel->getActiveSheet()
								...
								$data_recounts[$pids['pids'][$executive_id]['pid']][$pids['pids'][$executive_id]['isalter']][$rebid
										. '|' . $recs['is_support'] . '|'
										. $recs['support_dep']]['allcost'] = array(
										'row' => $now_line, 'col' => 16,
								...
								$data_recounts[$pids['pids'][$executive_id]['pid']][$pids['pids'][$executive_id]['isalter']][$rebid
										. '|' . $recs['is_support'] . '|'
										. $recs['support_dep']]['now_month_cost'] = array(
										'row' => $now_line, 'col' => 17,
										'value' => $now_month_cost);

								$now_pid_isalter = $pids['pids'][$executive_id]['pid']
										. '_'
										. $pids['pids'][$executive_id]['isalter'];
							}
							...
							$data_recounts[$pids['pids'][$executive_id]['pid']][$pids['pids'][$executive_id]['isalter']][$rebid
									. '|' . $recs['is_support'] . '|'
									. $recs['support_dep']]['cost_amount'] = array(
									'row' => $now_line, 'col' => 23,
								...
								foreach ($dr_array as $field_name => $amount_array) {
									...
									$objPHPExcel->getActiveSheet()
											->setCellValueByColumnAndRow(
													$amount_array['col'],
													$amount_array['row'],
													$prev_isalter === NULL ? $amount_array['value']
															: ($amount_array['value']
																	- $data_recount[$prev_isalter][$pKey][$field_name]['value']));
								}
							}
				...
				if (preg_match('/MSIE/', $ua) || preg_match('/Trident/', $ua)) {
					$filename = urlencode(
							date('YmdHis', $this->starttime) . '~'
									. date('YmdHis', $this->endtime) . '_财务大表')
							. '.xls';
				} else {
					$filename = date('YmdHis', $this->starttime) . '~'
							. date('YmdHis', $this->endtime) . '_财务大表.xls';
				}
