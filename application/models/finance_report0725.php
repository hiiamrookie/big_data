					...
					foreach ( $data_cost_cy [$month] as $executive_id => $contents ) {
						...
						foreach ( $contents as $nKey => $recs ) {
							...
							if ($now_pid_isalter !== ($pids ['pids'] [$executive_id] ['pid'] . '_' . $pids ['pids'] [$executive_id] ['isalter'])) {
								...
								$data_recounts [$pids ['pids'] [$executive_id] ['pid']] [$pids ['pids'] [$executive_id] ['isalter']] [$rebid . '|' . $recs ['is_support'] . '|' . $recs ['support_dep']] ['now_month_amount'] = array (
										'row' => $now_line,
										'col' => 15,
								...
								$data_recounts [$pids ['pids'] [$executive_id] ['pid']] [$pids ['pids'] [$executive_id] ['isalter']] [$rebid . '|' . $recs ['is_support'] . '|' . $recs ['support_dep']] ['now_month_cost'] = array (
										'row' => $now_line,
										'col' => 17,
